...
func populateActualModelNames(logs []*Log) {
	for _, log := range logs {
		...
		adminInfo, _ := values[logOtherAdminInfoKey].(map[string]any)
		if adminInfo == nil {
			continue
		}
		upstreamModel, _ := adminInfo["upstream_model_name"].(string)
		if upstreamModel != "" && upstreamModel != log.ModelName {
			log.ActualModelName = upstreamModel
		}
	}
}

// HideActualModelNames keeps the operational log response while removing the
// upstream model identity for viewers without the dedicated permission.
func HideActualModelNames(logs []*Log) {
	for _, log := range logs {
		...
		for _, scope := range []string{logOtherAdminInfoKey, logOtherRootInfoKey} {
			info, _ := values[scope].(map[string]any)
			if info == nil {
				continue
			}
			delete(info, "upstream_model_name")
			delete(info, "is_model_mapped")
			delete(info, "response_model")
		}
		// Older error logs stored mapping metadata at the top level. Remove the
		// legacy fields as well so a restricted viewer cannot recover the
		// upstream model from a pre-scoped record.
		for _, key := range []string{"upstream_model_name", "is_model_mapped", "response_model"} {
			delete(values, key)
		}
		log.Other = common.MapToJsonStr(values)
	}
}
