...
func BatchDeleteChannels(ids []int) (int64, error) {
	...
	for _, chunk := range lo.Chunk(ids, 200) {
		if err := tx.Where("channel_id in (?)", chunk).Delete(&ChannelErrorRecord{}).Error; err != nil {
			tx.Rollback()
			return 0, err
		}
...
func (channel *Channel) Delete() error {
	...
	if err := tx.Where("channel_id = ?", channel.Id).Delete(&ChannelErrorRecord{}).Error; err != nil {
		tx.Rollback()
		return err
	}
...
func DeleteChannelByStatus(status int64) (int64, error) {
	...
	if _, err := BatchDeleteChannels(ids); err != nil {
		return 0, err
	}
...
func (channel *Channel) ValidateSettings() error {
	...
	if channelOtherSettings.UpstreamErrorDisplay != nil {
		if err := channelOtherSettings.UpstreamErrorDisplay.Validate(); err != nil {
			return fmt.Errorf("invalid upstream_error_display: %w", err)
		}
	}
