...
func AppendRelayLogAdminInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, other *model.LogOther) {
	...
	if relayInfo != nil {
		if relayInfo.IsModelMapped {
			...
			if requestModel != "" && relayInfo.UpstreamModelName != "" && requestModel != relayInfo.UpstreamModelName {
				other.SetPublic("request_model_name", requestModel)
...
func AppendResponseModelLogInfo(relayInfo *relaycommon.RelayInfo, other *model.LogOther) {
	...
	visible := *observation
	if relayInfo.IsModelMapped {
		visible.RequestedModel = relayInfo.ClientModelName()
		visible.UpstreamModel = visible.RequestedModel
		visible.ReturnedModel = relaycommon.ClientVisibleErrorMessage(relayInfo, visible.ReturnedModel)
		other.SetAdmin("response_model", *observation)
	}
	other.SetPublic("response_model", visible)
}
