...
export function ChannelMutateDrawer({
  ...
  const onSubmit = useCallback(
    async (data: ChannelFormValues) => {
      ...
      if (isEditing && !canEditChannel) return
  ...
  const upstreamErrorDisplayFields = (
    <div
      role='group'
      aria-label={t('Upstream Error Display')}
      className={channelConfigurationBlockClassName(
        configuration.blocks.upstreamErrorDisplay,
        'flex flex-col gap-4'
      )}
    >
      <CardHeading
        title={t('Upstream Error Display')}
        status={configuration.blocks.upstreamErrorDisplay}
      ...
      <fieldset
        disabled={(isEditing && !canEditChannel) || isSubmitting}
        className='space-y-4 disabled:opacity-60'
      >
        <FormField
          control={form.control}
          ...
          render={({ field }) => (
            <FormItem className={sideDrawerSwitchItemClassName()}>
              <div className='space-y-0.5'>
                <FormLabel>
                  {t('Show sanitized upstream error details by default')}
                </FormLabel>
                <FormDescription>
                  {t(
        ...
        <div className='grid gap-4 sm:grid-cols-[10rem_1fr]'>
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t('Generic client status code')}</FormLabel>
                <FormControl>
                  <Input
                    type='number'
                    min={400}
                    max={599}
                    placeholder={String(DEFAULT_UPSTREAM_ERROR_STATUS_CODE)}
                    {...field}
                    onChange={(event) =>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t('Generic client error message')}</FormLabel>
                <FormControl>
                  <Textarea
                    rows={2}
                    maxLength={500}
                    placeholder={DEFAULT_UPSTREAM_ERROR_MESSAGE}
                    {...field}
                  />
                </FormControl>
                <FormDescription>
