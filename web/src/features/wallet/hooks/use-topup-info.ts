...
import { getManualWechatPaymentInfo } from '@/features/manual-payments/api'
...
export function useTopupInfo() {
  ...
  const fetchTopupInfo = useCallback(async () => {
    try {
      ...
      const [response, manualWechatResponse] = await Promise.all([
        getTopupInfo(),
        getManualWechatPaymentInfo().catch(() => ({
          success: false,
          data: undefined,
        })),
      ])
      ...
      const processedData: TopupInfo = {
        ...
        enable_manual_wechat_topup:
          manualWechatResponse.success &&
          manualWechatResponse.data?.enabled === true,
        manual_wechat_display_name:
          manualWechatResponse.data?.display_name || '',
        manual_wechat_expires_at: manualWechatResponse.data?.expires_at || 0,
      }
