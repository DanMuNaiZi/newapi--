...
describe('usage log access tier', () => {
  ...
  test('allows a delegated administrator with permission to use admin scope', () => {
    assert.equal(
      resolveLogsViewAccess(ROLE.AUTHORIZED_ADMIN, 'all', true),
      'admin'
    )
    assert.equal(
      resolveLogsViewAccess(ROLE.AUTHORIZED_ADMIN, 'all', false),
      'self'
    )
  })
})
