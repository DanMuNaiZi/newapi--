...
export function resolveLogsViewAccess(
  ...
  canManageScope = role >= ROLE.ADMIN
): LogsViewAccess {
  if (viewScope !== 'all' || !canManageScope) return 'self'
...
export function useLogsViewScope() {
  ...
  const { canViewGlobalLogs: canManageScope, canViewActualModel } =
    useUsageLogAccess()
  const viewAccess = resolveLogsViewAccess(role, viewScope, canManageScope)
