...
} from '@/features/system-settings/utils/access'
import {
...
export function useSidebarData(): SidebarData {
  ...
  const systemSettingsAccess = getSystemSettingsAccess(user)
  ...
  if (
    can(ADMIN_PERMISSION_RESOURCES.USER) ||
    can(ADMIN_PERMISSION_RESOURCES.SUBSCRIPTION)
  ) {
    adminItems.push({
  ...
  const systemSettingsLandingPath = getSystemSettingsLandingPath(user)
  if (systemSettingsLandingPath) {
    adminItems.push({
      title: t('System Settings'),
      url: systemSettingsLandingPath,
