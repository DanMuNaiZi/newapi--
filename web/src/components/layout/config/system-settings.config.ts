 ...
 */
export function getSystemSettingsNavGroups(
  t: TFunction,
  access: SystemSettingsSidebarAccess
): NavGroup[] {
  ...
  const authItems = getAuthSectionNavItems(t).filter((item) => {
    const isOAuthSection =
      item.url === '/system-settings/auth/oauth' ||
      item.url === '/system-settings/auth/custom-oauth'
    return isOAuthSection ? access.oauth : access.systemSettings
  })
  ...
  const billingItems = getBillingSectionNavItems(t).filter((item) => {
    const isPaymentSection = item.url === '/system-settings/billing/payment'
    return isPaymentSection ? access.payment : access.systemSettings
  })
