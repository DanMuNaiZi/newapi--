...
func optionPermission(key string, action string) authz.Permission {
	...
	if strings.HasPrefix(lowerKey, "payment_setting.") {
...
func isManualWechatPaymentOptionKey(key string) bool {
	switch strings.ToLower(strings.TrimSpace(key)) {
	case manualWechatEnabledOptionKey,
		manualWechatQRCodeOptionKey,
		manualWechatExpiresAtOptionKey,
		manualWechatInstructionsOptionKey:
		return true
	default:
		return false
	}
...
func GetOptions(c *gin.Context) {
	...
	for k, v := range common.OptionMap {
		...
		if isManualWechatPaymentOptionKey(k) {
			continue
		}
...
func UpdateOption(c *gin.Context) {
	...
	if isManualWechatPaymentOptionKey(option.Key) {
		common.ApiErrorMsg(c, "微信人工收款配置必须通过专用接口整体保存")
		return
	}
