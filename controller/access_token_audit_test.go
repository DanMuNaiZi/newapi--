		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, upgrade := range []bool{false, true} {
				t.Run(fmt.Sprintf("upgrade=%v", upgrade), func(t *testing.T) {
					...
					openedDatabases := []*gorm.DB{db}
					t.Cleanup(func() {
						closed := map[any]struct{}{}
						for _, opened := range openedDatabases {
							connection, err := opened.DB()
							if err != nil {
								continue
							}
							if _, ok := closed[connection]; ok {
								continue
							}
							closed[connection] = struct{}{}
							_ = connection.Close()
						}
					})
					...
					for range 2 {
						...
						openedDatabases = append(openedDatabases, model.DB)
						require.NoError(t, model.InitLogDB())
						openedDatabases = append(openedDatabases, model.LOG_DB)
					}
					if !upgrade {
