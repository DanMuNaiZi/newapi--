...
// 辅助函数
func rewriteResponseModelInJSONBytes(data []byte, info *relaycommon.RelayInfo) []byte {
	...
	if gjson.GetBytes(data, "model").Exists() {
		next, err := sjson.Set(rewritten, "model", model)
		if err != nil {
			return data
		}
		rewritten = next
	...
	if gjson.Get(rewritten, "response.model").Exists() {
		next, err := sjson.Set(rewritten, "response.model", model)
		if err != nil {
			return data
		}
		rewritten = next
...
func rewriteResponseModelInJSONString(data string, info *relaycommon.RelayInfo) string {
	rewritten := rewriteResponseModelInJSONBytes(common.StringToByteSlice(data), info)
	return string(rewritten)
}

func HandleStreamFormat(c *gin.Context, info *relaycommon.RelayInfo, data string, forceFormat bool, thinkToContent bool) error {
	info.SendResponseCount++
	data = rewriteResponseModelInJSONString(data, info)
...
func HandleFinalResponse(c *gin.Context, info *relaycommon.RelayInfo, lastStreamData string,
	...
	if responseModel := info.ResponseModelName(); responseModel != "" {
		model = responseModel
	}
